...
class Config(object):
    ...
    def set_config_CNN(self, num_of_filters, input_drop_prob, drop_prob, map_drop_prob, kernel_size=1):
        self.out_channels = num_of_filters
        self.convkb_drop_prob = drop_prob
        self.input_drop_prob = input_drop_prob
        self.map_drop = map_drop_prob
    ...
    def set_negative_slope(self, slope):
    ...
    def train_one_step(self, epoch):
        ...
        #     pred = self.trainModel.forward()
        #     loss = self.trainModel.loss(pred, to_var(y_binary))
        #     loss.backward()
        #     torch.nn.utils.clip_grad_norm_(self.trainModel.parameters(), 0.5)
        #     self.optimizer.step()
        #     return loss.item()
        # else:
        schedule = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=30, gamma=0.9)
        self.optimizer.zero_grad()
    ...
    def training_model(self):
        ...
        for epoch in training_range:
            ...
            for batch in range(self.nbatches):
                self.sampling()
                loss = self.train_one_step(epoch)
                res += loss
